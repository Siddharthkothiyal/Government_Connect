...
async function generateAiSummaries(profile, schemes) {
    if (schemes.length === 0) {
        return {
...
class EligibilityController {
    async checkEligibility(req, res) {
        ...
        const serviceResults = await eligibilityService.checkEligibility(data);
        const schemePairs = serviceResults.map((r) => ({
            dbScheme: r.scheme,
            eligibilityReason: r.eligibilityReason,
        }));
        const frontendSchemes = schemePairs.map((pair) => (0, transform_1.toFrontendScheme)(pair.dbScheme, pair.eligibilityReason));
