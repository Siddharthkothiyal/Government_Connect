...
Documents Required: ${scheme.documents.map((d: any) => d.documentName).join(', ')}
    ...
    const fallbackReply =
      schemes.length > 0
        ? `Based on your question "${data.message}", here are some relevant schemes you may want to check: ${schemes
            ...
            .join(
              ', '
            )}. For detailed eligibility, please fill the form on the Check Eligibility page. Always verify details on the official government website before applying.`
